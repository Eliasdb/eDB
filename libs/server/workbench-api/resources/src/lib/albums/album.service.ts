...
export class AlbumService {
  constructor(private readonly repo: AlbumRepo) {}
  ...
  async getOne(_ctx: RequestContext, id: string) {
    const { id: safe } = albumIdParamSchema.parse({ id });
    const found = await this.repo.getById(safe);
    if (!found) throw new NotFoundError('Album not found');
    return { album: found };
  }

  async create(ctx: RequestContext, body: CreateAlbumBody) {
    ensureLoggedIn(ctx);
    const created = await this.repo.create(body);
    return { album: created };
  }

  async update(ctx: RequestContext, id: string, body: UpdateAlbumBody) {
    ensureLoggedIn(ctx);
    const { id: safe } = albumIdParamSchema.parse({ id });
