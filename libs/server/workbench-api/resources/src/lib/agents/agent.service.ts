...
export class AgentService {
  ...
  async getOne(_ctx: RequestContext, id: string) {
    const { id: safe } = agentIdParamSchema.parse({ id });
    const found = await this.repo.getById(safe);
    if (!found) throw new NotFoundError('Agent not found');
    return { agent: found };
  }

  async create(ctx: RequestContext, body: CreateAgentBody) {
    ensureLoggedIn(ctx);
    const created = await this.repo.create(body);
    return { agent: created };
  }

  async update(ctx: RequestContext, id: string, body: UpdateAgentBody) {
    ensureLoggedIn(ctx);
    const { id: safe } = agentIdParamSchema.parse({ id });
