		...
		<section class="settings-page bg-gray-100">
			...
			<section class="sidenav">
				...
				<div class="sidenav-desktop">
					...
					<ui-sidenav
						[links]="links"
						(linkClick)="onLinkClick($event)"></ui-sidenav>
				</div>
			...
			<section class="settings-container" #settingsContainer>
				...
				<ui-structured-list
					...
					[uneditedMode]="true"
					[id]="group.id"
					[skeleton]="isLoading()"></ui-structured-list>
				}
...
export class ProfilePage implements OnInit, OnDestroy {
	...
	onLinkClick(clickedItem: LinkItem): void {
		this.links.forEach(
			(link) => (link.active = link.id === clickedItem.id)
		);
	...
	onSelectChange(newValue: string): void {
		...
		if (element) {
			...
			const headerOffset =
				15 *
				parseFloat(getComputedStyle(document.documentElement).fontSize); // 3rem in px
			if (container) {
	...
	private initializeComputedRows(): void {
		this.settingsGroups.forEach((group) => {
			this.computedRows[group.id] = computed(() =>
				this.computeRows(group)
			);
		});
	}
	...
	onUpdateEdit(groupId: string, rowIndex: number): void {
		...
		switch (field) {
			case 'Password':
				if (
					this.inputValues.newPassword !==
					this.inputValues.confirmPassword
				) {
