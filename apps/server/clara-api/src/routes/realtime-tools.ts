...
const route: FastifyPluginAsync = async (app) => {
  ...
  app.get('/realtime/tools', async (_req, reply) => {
    ...
    const instructions =
      'You are Clara. When the user asks to create/update/list tasks, contacts, or companies, ALWAYS call the correct tool immediately, then give a short confirmation. Keep replies concise.';
