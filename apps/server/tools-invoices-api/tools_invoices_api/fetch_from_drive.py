...
def get_folder_id_by_path(path_parts):
    results = service.files().list(
        q=f"name = '{path_parts[0]}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
    ...
    for part in path_parts[1:]:
        children = service.files().list(
            q=f"'{current_folder_id}' in parents and name = '{part}' and mimeType = 'application/vnd.google-apps.folder'",
            fields="files(id, name)", supportsAllDrives=True, includeItemsFromAllDrives=True
        ).execute().get("files", [])
        if not children:
