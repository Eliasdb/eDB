/* ------------------------------------------------------------------ */
/* contact-sidebar.component.ts – fixed, now wired to <ui-slide-in-sidebar> */
/* ------------------------------------------------------------------ */
      ...
      <div class="flex flex-col h-full border-l shadow-xl">
        ...
        <section class="p-6 overflow-y-auto flex-1">
          <ng-container *ngIf="!editMode(); else editTpl">
            ...
            <ng-container *ngIf="!showAdd(); else addFormTpl">
              <div
...
export class ContactSidebarComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    /* tiny delay so initial render isn't animated */
    setTimeout(() => (this.ready = true));
  }
  private ready = false;

  /* ------------ public API --------------- */
  /** Called by parent to show the sidebar */
  ...
  /* close with Esc in stand-alone mode */
